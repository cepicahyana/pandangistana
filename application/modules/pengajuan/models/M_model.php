...
//	$this->db->where("id_admin",$this->session->userdata("id"));
	...
	private function _get_datatables_peserta()
	{
		...
		$query="select * from v_peserta where sts_acc='".$sts_acc."' and sts_notif=0 ";
...
$dataForm=$this->getDataForm($dataEvent->id_form); 
        ...
        $this->load->library('excel');//Load library excelnya
			...
			for($iz=0;$iz<=$selisih;$iz++)
			{
				...
				if($jenis==3)
				{
					...
					for($x=1;$x<=2;$x++)
					{
						...
						$kodeIPsip=$additional.sprintf("%05s", $like->Auto_increment);
						 $IPsip=$kodeIPsip;
							...
							$cekdulu=$this->db->query("select * from data_peserta where kode_registrasi='".$IPsip."' and jenis='".$x."' ")->num_rows();
							if(!$cekdulu)
							{
								if($mode==2)
								{
					...
					$cekdulu=$this->db->query("select * from data_peserta where kode_registrasi='".$kodeIPsip."' and jenis='".$jenis."' ")->num_rows();
					if(!$cekdulu)
					{
						if($mode==2)
								{
	...
	$idevent=$this->idevent;
		...
		$file = $_FILES['userfile']['tmp_name'];
		 ...
		 {
			//$this->db->query("update data_peserta set status='2' where kode_registrasi IN (".$val->reg.") and gate='".$val->gate."' and jenis='".$val->jenis."'");
			$data=$val->reg;
			$da=explode(",",$data);
			$jml=count($da);
			foreach($da as $dim)
	...
	function setBlok()
	{
		...
		$berlaku	=	$this->input->get_post("berlaku");
		$jml		=	$this->input->get_post("jml");
		
		$data		=	$this->db->query("select * from v_peserta where nik='".$nik."' ")->row();
		...
		"jml_s_pagi"=>$data->jml_s_pagi,
		"jml_s_sore"=>$data->jml_s_sore,
		"jml_d_pagi"=>$data->jml_d_pagi,
		"jml_d_sore"=>$data->jml_d_sore,
		...
		"_ctime"=>date('Y-m-d H:i:s'),
		 ...
		 if($acara==1){
		     	$this->db->query("delete from data_peserta where nik='".$nik."' and jenis='1'
		 and (berlaku is null or berlaku='".$berlaku."') and (blok='".$blok."' or blok is null or blok='' )");
		 }else{
		     	$this->db->query("delete from data_peserta where nik='".$nik."' and jenis='2'
		 and (berlaku is null or berlaku='".$berlaku."') and (blok='".$blok."' or blok is null or blok='' )");
		 }
		
	    $this->db->query("delete from data_peserta where nik='".$nik."' and ( blok is null or blok='' ) ");
	   
		for($i=1;$i<=$jml;$i++)
		{   
			$this->db->set($data_awal);
			$this->db->set("nik",$nik);
			$this->db->set("jenis",$acara);
		...
		$sisa   =   $this->db->query("select * from data_peserta where nik='".$nik."' ")->num_rows();
		if(!$sisa){
		    	 
		    $this->db->set($data_awal); 
			$this->db->set("jenis",null);
			$this->db->set("blok",null);
			$this->db->set("berlaku",null);
			$this->db->set("_cid",$this->idu());
			$this->db->set("_ctime",date('Y-m-d H:i:s'));
			$this->db->insert("data_peserta");
		}
	...
	function getJmlBlok($nik,$jenis,$blok,$berlaku)
	{
		$this->db->where("jenis",$jenis);
		$this->db->where("nik",$nik);
		$this->db->where("blok",$blok);
		$this->db->where("berlaku",$berlaku);
		return $this->db->get("data_peserta")->num_rows();
	}
	function setStatus()
	{
